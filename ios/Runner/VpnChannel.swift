...
/// App-side bridge between Flutter (`app.greenvpn.ios/vpn`) and the
...
/// changes stream back on `app.greenvpn.ios/vpn_status`.
class VpnChannel: NSObject, FlutterStreamHandler {

    private let extensionBundleId = "app.greenvpn.ios.PacketTunnel"
    private var eventSink: FlutterEventSink?
    private var manager: NETunnelProviderManager?

    static func register(with registrar: FlutterPluginRegistrar) {
        let instance = VpnChannel()
        let method = FlutterMethodChannel(name: "app.greenvpn.ios/vpn",
                                          binaryMessenger: registrar.messenger())
        method.setMethodCallHandler(instance.handle)
        let status = FlutterEventChannel(name: "app.greenvpn.ios/vpn_status",
                                         binaryMessenger: registrar.messenger())
